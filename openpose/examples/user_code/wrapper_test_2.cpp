...
class WUserInput2 : public op::WorkerProducer<std::shared_ptr<std::vector<UserDatum>>>
{
public:
    WUserInput2(const std::string& filePath) :
        mCounter{0},
        _filePath{filePath}
    {
    }
    ...
    std::shared_ptr<std::vector<UserDatum>> workProducer()
    {
        try
        {
            ...
            //これを他のスレッドと共有している！！
            //これを複数用意！
            auto datumsPtr2 = std::make_shared<std::vector<UserDatum>>();
            //datumsPtr2 = std::make_shared<std::vector<UserDatum>>();
            datumsPtr2->emplace_back();
            auto& datum = datumsPtr2->at(0);
            ...
            //std::cout<<"counter"<<mCounter<<std::endl;
            //int size = datumsPtr2->size();
            //std::cout<<"datumsPtr2.size : "<<size<<std::endl;
            op::log("WUserInput2");
            return datumsPtr2;
        }
        catch (const std::exception& e)
...
// This worker will just read and return all the jpg files in a directory
class WUserOutput2 : public op::WorkerConsumer<std::shared_ptr<std::vector<UserDatum>>>
{
public:
    void initializationOnThread() {}

    void workConsumer(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr2)
    {
        try
        {
            // User's displaying/saving/other processing here
                // datum.cvOutputData: rendered frame with pose or heatmaps
                // datum.poseKeypoints: Array<float> with the estimated pose
            if (datumsPtr2 != nullptr && !datumsPtr2->empty())
            {
                // Show in command line the resulting pose keypoints for body, face and hands
                //op::log("\nKeypoints:");
                // Accesing each element of the keypoints
                const auto& poseKeypoints = datumsPtr2->at(0).poseKeypoints;
                //op::log("Person pose keypoints:");
                for (auto person = 0 ; person < poseKeypoints.getSize(0) ; person++)
                {
                    //op::log("Person " + std::to_string(person) + " (x, y, score):");
                ...
                //op::log(" ");
                // Alternative: just getting std::string equivalent
                //op::log("Face keypoints: " + datumsPtr2->at(0).faceKeypoints.toString());
                //op::log("Left hand keypoints: " + datumsPtr2->at(0).handKeypoints[0].toString());
                //op::log("Right hand keypoints: " + datumsPtr2->at(0).handKeypoints[1].toString());

                // Display rendered output image
                op::log("WUserOutput2");
                cv::imshow("User worker GUI2", datumsPtr2->at(0).cvOutputData);
                cv::waitKey(1); // It displays the image and sleeps at least 1 ms (it usually sleeps ~5-10 msec to display the image)
            }
        }
        catch (const std::exception& e)
...
int openPoseTutorialWrapper2(char* argv[])
{
    ...
    auto wUserInput = std::make_shared<WUserInput>(argv[2]);
    auto wUserInput2 = std::make_shared<WUserInput2>(argv[3]);
    // Initializing the user custom classes
    // Frames producer (e.g. video, webcam, ...)
    //auto wUserInput = std::make_shared<WUserInput>(FLAGS_image_dir);
    // GUI (Display)
    auto wUserOutput = std::make_shared<WUserOutput>();
    auto wUserOutput2 = std::make_shared<WUserOutput2>();

    op::Wrapper<std::vector<UserDatum>> opWrapper;
    op::Wrapper<std::vector<UserDatum>> opWrapper2;
    // Add custom input
    const auto workerInputOnNewThread = true;
    const auto workerInputOnNewThread2 = true;
    opWrapper.setWorkerInput(wUserInput, workerInputOnNewThread);
    opWrapper2.setWorkerInput(wUserInput2, workerInputOnNewThread2);
    
   // Add custom output
    const auto workerOutputOnNewThread = true;
    const auto workerOutputOnNewThread2 = true;
    opWrapper.setWorkerOutput(wUserOutput, workerOutputOnNewThread);
    opWrapper2.setWorkerOutput(wUserOutput2, workerOutputOnNewThread2);
    // Configure OpenPose
    ...
    opWrapper2.exec();  // It blocks this thread until all threads have finished
