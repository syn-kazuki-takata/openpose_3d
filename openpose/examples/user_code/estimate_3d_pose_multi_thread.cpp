...
class Estimate2dPose
{
public:
    /*
    Estimate2dPose(cv::VideoCapture& camera,
                    int frameNum,
                    cv::Mat& mapx,
                    cv::Mat& mapy,
                    ...
                    std::vector<std::vector<cv::Point2d>>& bodyPoints)
    {
        ...
        _mapx = mapx;
        _mapy = mapy;
        _cvMatToOpInput = &cvMatToOpInput;
        _cvMatToOpOutput = &cvMatToOpOutput;
        _poseExtractorCaffe = &oseExtractorCaffe;
    ...
    */
    Estimate2dPose(cv::VideoCapture& camera,
        ...
        _cvMatToOpInput{cvMatToOpInput},
        _cvMatToOpOutput{cvMatToOpOutput},
        _poseExtractorCaffe{poseExtractorCaffe},
        _bodyPoints{bodyPoints}
        {
        }
    ...
    void workProducer()
    {
        try
        {
            for(int video_framePtr=0; video_framePtr<_frameNum; video_framePtr++){
                cv::Mat frame, undistorted_image;
                _camera >> frame;
                cv::remap(frame, undistorted_image, _mapx, _mapy, INTER_LINEAR);
...
private:
    ...
    cv::Mat _mapx;
    cv::Mat _mapy;
    op::CvMatToOpInput& _cvMatToOpInput;
    op::CvMatToOpOutput& _cvMatToOpOutput;
    op::PoseExtractorCaffe& _poseExtractorCaffe;
    std::vector<std::vector<cv::Point2d>> _bodyPoints;
};
...
int main(int argc, char *argv[])
{
    ...
    std::vector<std::vector<std::vector<cv::Point2d>>> pose_2d_vec_each_camera(camera_value); //pose_2d_vec_each_camera[camera_num][frame_num][joint_num]
    ...
    for(int i=0; i<camera_value; i++){
        TypedefWorker Estimate2dPose = std::make_shared<Estimate2dPose>(cameras[i],
                                                                        frameNum,
                                                                        mapx[i],
                                                                        mapy[i],
                                                                        cvMatToOpInput,
                                                                        ...
                                                                        poseExtractorCaffe,
                                                                        bodyPoints[i]);
        threadManager.add(threadId++, Estimate2dPose, queueIn++, queueOut++);       // スレッドに二次元座標推定の追加
    }
    */
    TypedefWorker Estimate2dPose0 = std::make_shared<Estimate2dPose>(cameras[0],
                                                                        frameNum,
                                                                        mapx[0],
                                                                        mapy[0],
                                                                        cvMatToOpInput,
                                                                        cvMatToOpOutput,
                                                                        poseExtractorCaffe,
                                                                        pose_2d_vec_each_camera[0]);
    threadManager.add(threadId++, Estimate2dPose0, queueIn++, queueOut++);       // スレッドに二次元座標推定の追加

    TypedefWorker Estimate2dPose1 = std::make_shared<Estimate2dPose>(cameras[1],
                                                                        frameNum,
                                                                        mapx[1],
                                                                        mapy[1],
                                                                        cvMatToOpInput,
                                                                        cvMatToOpOutput,
                                                                        poseExtractorCaffe,
                                                                        pose_2d_vec_each_camera[1]);
    threadManager.add(threadId++, Estimate2dPose1, queueIn++, queueOut++);       // スレッドに二次元座標推定の追加
