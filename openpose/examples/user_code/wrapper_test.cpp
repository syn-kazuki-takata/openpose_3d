...
public:
    ...
    std::shared_ptr<std::vector<UserDatum>> workProducer()
    {
        try
        {
            ...
            //datumsPtr = std::make_shared<std::vector<UserDatum>>();
            datumsPtr->emplace_back();
            auto& datum = datumsPtr->at(0);

            // Fill datum
            //datum.cvInputData = cv::imread(mImageFiles.at(mCounter++));
            cv::VideoCapture cap(_filePath);
            cap.set(CV_CAP_PROP_POS_FRAMES, mCounter++);
            cv::Mat frame;
            cap >> frame;
            ...
            //int size = datumsPtr->size();
            //std::cout<<"datumsPtr.size : "<<size<<std::endl;
            return datumsPtr;
        }
...
private:
    ...
    //std::shared_ptr<std::vector<UserDatum>> datumsPtr;
};
...
{
public:
    WUserOutput(int _camera_ptr, std::shared_ptr<std::vector<UserDatum>>& _datums_ptr) :
    //WUserOutput(int _camera_ptr, std::shared_ptr<std::vector<WUserInput>>& __datumsPtr) :
        // mImageFiles{op::getFilesOnDirectory(directoryPath, std::vector<std::string>{"jpg", "png"})}, // If we want "jpg" + "png" images
        camera_ptr{_camera_ptr},
        datums_ptr{_datums_ptr}
    {
    }
    ...
    void workConsumer(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr){
        try
        {
            ...
            {
                if(!datums_ptr->at(0).cvInputData.empty()){

                    std::cout<<"datums_ptr is not empty"<<std::endl;
                    // Show in command line the resulting pose keypoints for body, face and hands
                    op::log("\nKeypoints:");
                    // Accesing each element of the keypoints
                    
                    const auto& poseKeypoints = datums_ptr->at(0).poseKeypoints;
                    ...
                    op::log("Face keypoints: " + datums_ptr->at(0).faceKeypoints.toString());
                    op::log("Left hand keypoints: " + datums_ptr->at(0).handKeypoints[0].toString());
                    op::log("Right hand keypoints: " + datums_ptr->at(0).handKeypoints[1].toString());
                    /*
                    // Display rendered output image
                    std::string window_name = "camera_" + std::to_string(camera_ptr);
                    if (datums_ptr != nullptr && !datums_ptr->empty()){
                        std::cout<<"not empty"<<std::endl;
                        for(auto& datum : *datums_ptr){
                            cv::imshow(window_name, datum.cvOutputData);
                        }
                    }
                    */
                    std::string window_name = "camera_" + std::to_string(camera_ptr);
                    op::log(window_name);
                    cv::imshow(window_name, datums_ptr->at(0).cvInputData);
...
private:
    int camera_ptr;
    std::shared_ptr<std::vector<UserDatum>> datums_ptr;
    //std::shared_ptr<std::vector<WUserInput>>& _datumsPtr;
};

// This worker will just read and return all the jpg files in a directory
class WUserOutput_ : public op::WorkerConsumer<std::shared_ptr<std::vector<UserDatum>>>
{
public:
    WUserOutput_(int _camera_ptr) :
    //WUserOutput(int _camera_ptr, std::shared_ptr<std::vector<WUserInput>>& __datumsPtr) :
        // mImageFiles{op::getFilesOnDirectory(directoryPath, std::vector<std::string>{"jpg", "png"})}, // If we want "jpg" + "png" images
        camera_ptr{_camera_ptr}
    {
        ...
        try
        {
            ...
            if (datumsPtr != nullptr && !datumsPtr->empty())
            {
                if(!datumsPtr->at(0).cvInputData.empty()){
                    ...
                    std::cout<<"datumsPtr is not empty"<<std::endl;
                    // Show in command line the resulting pose keypoints for body, face and hands
                    op::log("\nKeypoints:");
                    // Accesing each element of the keypoints
                    ...
                    */
                    std::string window_name = "camera_" + std::to_string(camera_ptr);
                    op::log(window_name);
                    cv::imshow(window_name, datumsPtr->at(0).cvInputData);
                    cv::waitKey(1); // It displays the image and sleeps at least 1 ms (it usually sleeps ~5-10 msec to display the image)
                }
            }
        }
...
int openPoseTutorialWrapper2(char *argv[])
{
    // logging_level
    ...
    std::vector<std::shared_ptr<std::vector<UserDatum>>> datums_ptrs(camera_num);
    std::vector<std::shared_ptr<WUserInput>> wUserInputs(camera_num);
    std::vector<std::shared_ptr<WUserOutput_>> wUserOutputs(camera_num);
    for(int i=0; i<camera_num; i++){
        datums_ptrs[i] = std::make_shared<std::vector<UserDatum>>();
        //auto wUserInput = std::make_shared<WUserInput>(argv[2+i], datums_ptrs[i]);
        auto wUserInput = std::make_shared<WUserInput>(argv[2+i]);
        wUserInputs[i] = wUserInput;
        //std::vector<std::shared_ptr<UserDatum>> _wUserInput = (std::vector<std::shared_ptr<UserDatum>>)wUserInput;
        //auto wUserOutput = std::make_shared<WUserOutput>(i, datums_ptrs[i]);
        auto wUserOutput = std::make_shared<WUserOutput_>(i);
        wUserOutputs[i] = wUserOutput;
    }
    // Processing
    ...
    /*
    for(int i=0; i<camera_num; i++){
        opWrapper.setWorkerInput(wUserInputs[i], workerInputOnNewThread);
        opWrapper.setWorkerOutput(wUserOutputs[i], workerOutputOnNewThread);
    }
    */
    opWrapper.setWorkerInput(wUserInputs[0], workerInputOnNewThread);
    opWrapper.setWorkerInput(wUserInputs[1], workerInputOnNewThread);
    opWrapper.setWorkerOutput(wUserOutputs[0], workerOutputOnNewThread);
...
int main(int argc, char *argv[])
{
    ...
    return openPoseTutorialWrapper2(argv);
}
