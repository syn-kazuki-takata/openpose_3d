...
std::vector<cv::Point2f> internal::find_chessboard_corners(const cv::Mat& image, int rows, int cols) {
    ...
    cv::waitKey(1);
    std::cout<<"find_corners"<<std::endl;
    return corners;
}

// undistorts an fisheye image
// @ref http://stackoverflow.com/questions/38983164/opencv-fisheye-undistort-issues
cv::Mat fisheye_calibration::undistort(const cv::Mat& K, const cv::Mat& D, const cv::Mat& frame) {
...
// main
std::vector<cv::Mat> fisheye_calibration::fisheye_calibration_(std::vector<cv::Mat> images) {
    ...
    const double chess_size = 40.0 / 1000.0;
    ...
    std::cout<<"detect frame : "<<image_points.size()<<std::endl;
    ...
    */
    std::vector<cv::Mat> return_vec = {intrinsic, distortion};
    return return_vec;
}
